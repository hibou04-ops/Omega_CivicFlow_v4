...
MIN_TEXT_LENGTH = 100
MIN_CHUNK_LENGTH = 150  # v4: 80 → 150 (orphan 제거하되 3행 재무비율표 보존)
...
def chunk_text_quality(text: str, meta: Dict = None) -> List[str]:
    ...
    # ── 2단계: 청크 크기 상수 (v4: 테이블 전용 상향) ──
    NARRATIVE_TARGET_SIZE = 900
    TABLE_TARGET_SIZE = 1800
    OVERLAP_SIZE = 120
    all_chunks = []
    ...
    for section_title, section_text in sections:
        ...
        # ── v4: 3-Phase 라인 태깅 (label-value 결합 보장) ──
        # Phase 1: 각 라인 분류 (numeric / label_candidate / narrative / blank)
        tagged = []
        for line in section_text.split('\n'):
            ...
            wide_gaps = len(re.findall(r'  +', stripped))
            ...
            if is_numeric_row:
                tagged.append({'text': stripped, 'type': 'numeric'})
            elif len(stripped) <= 30 and digit_count <= 3:
                tagged.append({'text': stripped, 'type': 'label_candidate'})
            else:
                tagged.append({'text': stripped, 'type': 'narrative'})

        # Phase 2: label_candidate 를 인접 numeric 행에 흡수 (핵심 수정)
        def _nearest_non_blank(idx, direction, window=3):
            i = idx + direction
            steps = 0
        ...
        for i, t in enumerate(tagged):
            if t['type'] == 'label_candidate':
                nxt = _nearest_non_blank(i, +1)
                prv = _nearest_non_blank(i, -1)
                if nxt == 'numeric' or prv == 'numeric':
                    tagged[i]['type'] = 'numeric'
                else:
                    tagged[i]['type'] = 'narrative'

        # Phase 3: 연속된 동일 type 을 블록으로 그룹핑
        table_blocks = []
        narrative_blocks = []
        cur_buf = []
        cur_type = None
            ...
            if cur_type is None:
                ...
                if joined:
                    if cur_type == 'numeric':
                        ...
                        narrative_blocks.append(joined)
                cur_type = t['type']
                cur_buf = [t['text']]
        ...
        for table in table_blocks:
            full_table_chunk = prefix + table
            if len(full_table_chunk) < MIN_CHUNK_LENGTH:
                continue
            if len(full_table_chunk) <= TABLE_TARGET_SIZE * 1.2:
                all_chunks.append(full_table_chunk)
            else:
                rows = table.split('\n')
                if len(rows) > 4:
                    table_header = '\n'.join(rows[:2]) + '\n'
                ...
                sub_prefix = prefix + table_header
                current = sub_prefix
                for row in data_rows:
                    if len(current) + len(row) > TABLE_TARGET_SIZE:
        ...
        for sent in sentences:
            if len(sent) < 5:
                continue
            if len(current_chunk) + len(sent) > NARRATIVE_TARGET_SIZE:
